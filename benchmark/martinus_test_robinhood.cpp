...
void bench_sequential_insert(HS& r, MicroBenchmark& mb, const std::string& title, size_t increase, size_t totalTimes, std::vector<std::vector<Stats>>& all_stats) {
    ...
    for (int ti = 0; ti < static_cast<int>(times); ++ti) {
        ...
        for (size_t up = 0; up < static_cast<size_t>(upTo); ++up) {
          i = data[up];
          r[static_cast<typename HS::key_type>(i)] = static_cast<typename HS::mapped_type>(i);
        }
...
template<class HS>
void bench_batch_insert_robinhood(HS& r, MicroBenchmark& mb, const std::string& title,
		size_t increase, size_t totalTimes, std::vector<std::vector<Stats>>& all_stats, bool est = true) {
    std::cout << title << "; ";
    std::cout.flush();
    std::vector<Stats> stats;
    Stats s;
    ...
    std::vector<typename HS::value_type> data(upTo);
    std::vector<typename HS::key_type> keys(upTo);
    std::vector<typename HS::key_type> rand_keys(upTo);
    ...
    for (int ti = 0; ti < static_cast<int>(times); ++ti) {
        ...
        for (size_t up = 0; up < static_cast<size_t>(upTo); ++up) {
          i = rand();
            ...
            data[up].second = static_cast<typename HS::mapped_type>(i);
          keys[up] = static_cast<typename HS::key_type>(i);
          rand_keys[up] = static_cast<typename HS::key_type>(rand());
        }
        t.restart();
        if (est)
        	r.insert(data);
        else
        	r.insert_no_estimate(data);
        s.elapsed_insert = t.elapsed() / upTo;
        auto gm = get_mem();
        s.mem = gm - mem_before;
        if (gm < mem_before) {
        ...
        // query existing
        while (mb.keepRunning()) {
          found += r.find(results.data(), keys.data(), keys.data() + keys.size());
        }
        s.elapsed_find_existing = mb.min() / upTo;

        // query nonexisting
        while (mb.keepRunning()) {
          found += r.find(results.data(), rand_keys.data(), rand_keys.data() + keys.size());
        }
        s.elapsed_find_nonexisting = mb.min() / upTo;
        s.found = found;
        stats.push_back(s);
...
template<class HS>
void bench_batch_insert_radix(HS& r, MicroBenchmark& mb, const std::string& title, size_t increase, size_t totalTimes, std::vector<std::vector<Stats>>& all_stats) {
    std::cout << title << "; ";
    std::cout.flush();
    std::vector<Stats> stats;
    Stats s;
    ...
    for (int ti = 0; ti < static_cast<int>(times); ++ti) {
        ...
        // query nonexisting
        while (mb.keepRunning()) {
          found += r.find(rand_keys.data(), rand_keys.size(), results.data());
        }
        s.elapsed_find_nonexisting = mb.min() / upTo;
        s.found = found;
        stats.push_back(s);
...
template<class HS>
void bench_batch_insertonly_radix(HS& r, MicroBenchmark& mb, const std::string& title,
		size_t increase, size_t totalTimes, std::vector<std::vector<Stats>>& all_stats, bool est = true) {
    std::cout << title << "; ";
    std::cout.flush();
    std::vector<Stats> stats;
    ...
    for (int ti = 0; ti < static_cast<int>(times); ++ti) {
        ...
        r.insert(keys.data(), keys.size());
        if (ti == static_cast<int>(times - 1)) r.finalize_insert();
...
//    }
    {
        ...
        googlemap.min_load_factor(min_load);
        bench_sequential_insert(googlemap, mb, "google::dense_hash_map", upTo, times, all_stats);
    }
...
template<class HS>
void bench_sequential_erase(HS& r, MicroBenchmark& mb, const std::string& title, size_t increase, size_t totalTimes, std::vector<std::vector<Stats>>& all_stats) {
    std::cout << title << "; ";
    std::cout.flush();
    std::vector<Stats> stats;
    Stats s;
...
template<class HS>
void bench_sequential_erase_pair(HS& r, MicroBenchmark& mb, const std::string& title, size_t increase, size_t totalTimes, std::vector<std::vector<Stats>>& all_stats) {
    std::cout << title << "; ";
    std::cout.flush();
    std::vector<Stats> stats;
    Stats s;
...
template<class HS>
void bench_batch_erase(HS& r, MicroBenchmark& mb, const std::string& title, size_t increase, size_t totalTimes, std::vector<std::vector<Stats>>& all_stats) {
    std::cout << title << "; ";
    std::cout.flush();
    std::vector<Stats> stats;
    Stats s;
...
// find returns mapped_types only.
template<class HS>
void bench_batch_erase_robinhood(HS& r, MicroBenchmark& mb, const std::string& title, size_t increase, size_t totalTimes, std::vector<std::vector<Stats>>& all_stats) {
    std::cout << title << "; ";
    std::cout.flush();
    std::vector<Stats> stats;
    Stats s;
...
template<class HS>
void bench_batch_erase_radix(HS& r, MicroBenchmark& mb, const std::string& title, size_t increase, size_t totalTimes, std::vector<std::vector<Stats>>& all_stats) {
    std::cout << title << "; ";
    std::cout.flush();
    std::vector<Stats> stats;
    Stats s;
...
template<class K, class V, template <typename> class H, template <typename> class Havx >
std::vector<std::vector<Stats>> bench_insert_erase(size_t upTo, size_t times, double max_load, double min_load) {
    std::cout << "Title;1M inserts [sec];find 1M existing [sec];find 1M nonexisting [sec];memory usage [MB];foundcount" << std::endl;
    std::vector<std::vector<Stats>> all_stats;
...
#if defined(ENABLE_PREFETCH)
	{
		...
		m.set_max_load_factor(max_load);
		m.set_min_load_factor(min_load);
		bench_sequential_erase_pair(m, mb, "RH", upTo, times, all_stats);
	}
	{
		::fsc::hashmap_robinhood_doubling<K, V, H<K>> m;
		m.set_max_load_factor(max_load);
		m.set_min_load_factor(min_load);
		bench_batch_erase(m, mb, "BRH", upTo, times, all_stats);
	}
#endif

	{
		...
		m.set_max_load_factor(max_load);
		m.set_min_load_factor(min_load);
		bench_batch_erase(m, mb, "BRH_Prefetch", upTo, times, all_stats);
	}
    {
        ::fsc::hashmap_robinhood_offsets<K, V, H> m;
		m.set_max_load_factor(max_load);
		m.set_min_load_factor(min_load);
        bench_batch_erase_robinhood(m, mb, "BRHO_Prefetch", upTo, times, all_stats);  // no overflow
    }
    {
        ::fsc::hashmap_robinhood_offsets<K, V, Havx> m;
		m.set_max_load_factor(max_load);
		m.set_min_load_factor(min_load);
        bench_batch_erase_robinhood(m, mb, "BRHO_Prefetch_avx", upTo, times, all_stats);  // no overflow
    }
    {
        ::fsc::hashmap_radixsort<K, V, H> m;
        bench_batch_erase_radix(m, mb, "BRS_Prefetch", upTo, times, all_stats);  // radixsort
    }

    {
        ::fsc::hashmap_radixsort<K, V, Havx> m;
        bench_batch_erase_radix(m, mb, "BRS_Prefetch_avx", upTo, times, all_stats);  // radixsort
    }
#if defined(ENABLE_PREFETCH)

    {
        RobinHoodInfobytePairNoOverflow::Map<K, V, H<K>> m;
		m.max_load_factor(max_load);
        bench_sequential_erase(m, mb, "RobinHoodInfobytePairNoOverflow", upTo, times, all_stats);
    }
...
////    }
    ...
    m.set_max_load_factor(max_load);
    m.set_min_load_factor(min_load);
        bench_sequential_erase_pair(m, mb, "LP", upTo, times, all_stats);
    }
    {
        ::fsc::hashmap_linearprobe_doubling<K, V, H<K>> m;
    m.set_max_load_factor(max_load);
    m.set_min_load_factor(min_load);
        bench_batch_erase(m, mb, "LP_BATCH", upTo, times, all_stats);
    }
...

    {
        ...
        googlemap.max_load_factor(max_load);
        googlemap.min_load_factor(min_load);
        bench_sequential_erase(googlemap, mb, "google::dense_hash_map", upTo, times, all_stats);
    }
...
////    }
...

    {
        std::unordered_map<K, V, H<K>> m;
        m.max_load_factor(max_load);
        bench_sequential_erase(m, mb, "std::unordered_map", upTo, times, all_stats);
    }
////    {
////        std::unordered_map<int, int, H<K>> m;
////        m.max_load_factor(0.5f);
