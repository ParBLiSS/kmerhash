...
/**
 * @file    hybrid_batched_radixsort_map.hpp
 * @ingroup index
...
#define HYBRID_BATCHED_RADIXSORT_MAP_HPP
...
namespace hsc  // hybrid std container
{
  ...
  /**
   * @brief  hybrid radixsort map following some of std unordered map's interface.
   * @details
   *         it has as much of the same methods of distributed_batched_radixsort_map as possible.  however, all methods consider the fact
   ...
   *
   *         This allows the possibility of using hybrid radixsort map as local storage for coarser grain distributed container.
   *
   *         Note that communicator requires a mapping strategy between a key and the target processor/thread/partition.  The mapping
   *         may be done using a hash, similar to the local hybrid radixsort map, or it may be done via sorting/lookup or other mapping
...
// 
    ...
    protected:
      ...
      /**
       * @brief query new elements in the hybrid batched_radixsort_multimap.
       * @param input  vector.  will be permuted.
       */
...
//      }
      ...
      /**
       * @brief find elements with the specified keys in the hybrid batched_radixsort_multimap.
  ...
  /**
   * @brief  hybrid radixsort map following std radixsort map's interface.
  ...
  /**
   * @brief  hybrid radixsort reduction map following std radixsort map's interface.  Insertion applies the binary reduction operator between the existing and inserted element (in that order).
  ...
  /**
   * @brief  hybrid radixsort counting map following std radixsort map's interface.  Insertion applies the binary reduction operator between the existing and inserted element (in that order).
   * @details   This class is modeled after the hashmap_batched_radixsort_doubling_offsets, but allows a binary reduction operator to be used during insertion.
   *
   *         the reduction operator is not assumed to be associative.  The operator is called with parameters existing element, then new element to insert.
   *
   ...
   *         as such, communicator may be defined for MPI, UPC, OpenMP, etc.
   *
   *         This allows the possibility of using hybrid radixsort map as local storage for coarser grain hybrid container.
   *
   *         Note that communicator requires a mapping strategy between a key and the target processor/thread/partition.  The mapping
   *         may be done using a hash, similar to the local hybrid radixsort map, or it may be done via sorting/lookup or other mapping
   *         mechanisms.  The choice may be constrained by the communication approach, e.g. global sorting  does not work well with
...
#endif // HYBRID_BATCHED_RADIXSORT_MAP_HPP
