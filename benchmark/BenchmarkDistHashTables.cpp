...
class special_keys {
    ...
    public:
        ...
        inline constexpr T get_splitter() {   // 10000000000000
            return ~(max >> 1);
        }
...
void read_key_val_pairs(std::vector<std::pair<K, V>> & output, size_t const & global_count, std::string filename, mxx::comm const & comm) {
        ...
        if (comm.rank() == 0) {
            ...
            pSize = std::min(pSize, global_count / comm.size());
            if (pSize < (global_count / comm.size())) printf("input file has to few elements to produce %ld per process.  actual %ld\n", global_count, pSize);
...
void generate_unique_key_val_pairs(std::vector<std::pair<K, V> > & output,
    ...
    if (lcount < count) printf("too few elements in unique list to produce %ld per process.  actual %ld\n", count, lcount);
...
void generate_key_val_pairs(std::vector<std::pair<K, V> > & output,
    ...
    if (second) std::advance(iter, uniques.size() >> 1);
    ValType val = 1;
    size_t el_cnt = 0;
...
//    TCLAP::ValueArg<std::string> queryArg("Q", "query", "FASTQ file path for query. default to same file as index file", false, "", "string", cmd);
  ...
  if (comm.rank() == 0) {
    ...
    std::cout << "      \tDistTrans=" << typeid(DistTrans<KeyType>).name() << std::endl;
  ...
  BL_BENCH_INIT(test);
  {
	  ...
	  BL_BENCH_COLLECTIVE_START(test, "get_keys", comm);
      get_unique_keys(unique_keys, filename, global_count, comm);
	  BL_BENCH_END(test, "get_keys", unique_keys.size());

#if (pINDEX == COUNT)
	  BL_BENCH_COLLECTIVE_START(test, "get_input", comm);
      if (repeats == 1.0) generate_unique_keys(input, unique_keys, count, false);
      else generate_keys(input, unique_keys, count, repeats, false);
	  BL_BENCH_END(test, "get_input", input.size());
#else
	  BL_BENCH_COLLECTIVE_START(test, "get_keyvals", comm);
      if (repeats == 1.0) generate_unique_key_val_pairs(input, unique_keys, count, false);
      else generate_key_val_pairs(input, unique_keys, count, repeats, false);
	  BL_BENCH_END(test, "get_keyvals", input.size());
#endif
...

  {
    ...
    std::vector<KeyType> inserted;
	  ...
	  BL_BENCH_COLLECTIVE_START(test, "get_inserted", comm);
      for (auto x : input) {
          inserted.emplace_back(x.first);
      }
	  BL_BENCH_END(test, "get_inserted", inserted.size());
#endif

	  ::std::vector<KeyType > missing;
	  BL_BENCH_COLLECTIVE_START(test, "get_missing", comm);
      if (repeats == 1.0) generate_unique_keys(missing, unique_keys, count * missing_frac + 1, true);
      else generate_keys(missing, unique_keys, count * missing_frac + 1, repeats, true);
	  BL_BENCH_END(test, "get_missing", missing.size());

        BL_BENCH_COLLECTIVE_START(test, "get_query", comm);
#if (pINDEX == COUNT)
        ...
        BL_BENCH_END(test, "get_query", query.size());
    }
