...
do
	...
	strand=$(echo $f | sed 's/kmc2-\(.*\)-k[0-9]*-t[^-]*[^\.]*\.[0-9]*\.log/\1/')
	k=$(echo $f | sed 's/kmc2-.*-k\([0-9]*\)-t[^-]*[^\.]*\.[0-9]*\.log/\1/')
	threads=$(echo $f | sed 's/kmc2-.*-k[0-9]*-t\([^-]*\)[^\.]*\.[0-9]*\.log/\1/')
	iter=$(echo $f | sed 's/kmc2-.*-k[0-9]*-t[^-]*[^\.]*\.\([0-9]*\)\.log/\1/')

	if grep -e "1000genome" $f > /dev/null;
	then 
		dataset=K1
	else
		dataset=$(echo $f | sed 's/kmc2-.*-k[0-9]*-t[^-]*-\(.*\)\.[0-9]*\.log/\1/')
	fi


	echo "kmc2,${format},4,${k},${strand},1,${threads},${dataset},${iter},$(summarize_times $f)"
done
