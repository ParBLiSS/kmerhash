...
source /home/tpan/scripts/gcc-mpich.sh

which mpirun

BIN_DIR=/home/tpan/build/kmerhash-gcc-mpich/bin
...
MPIRUN_CMD="/shared/hpc/mpich/bin/mpirun"
...
then
  ...
  NEW_NUM_NODES=${NUM_NODES}
	...
	$MPIRUN_CMD -np $NP -errfile-pattern=$bmlogdir/bm-err-$NP-%r.log -outfile-pattern=$bmlogdir/bm-out-$NP-%r.log $BM $NUM_VOTE_OFF $GOOD_HOSTFILE
...
for s in `cat $GOOD_HOSTFILE | sort | uniq`; do printf "$s:${PPN}\n"; done > $TFILE
...
# kmerind
	...
	OUT_PREFIX=${logdir}/kh-mpich-${MAP}-${HASH}-${dataset}-L${l}-P${t}.${iter}
  ...
  then
    	...
    	# command to execute
   	cmd="$MPIRUN_CMD -np $t -f $TFILE --map-by core --bind-to core $EXEC --max_load $l --min_load 0.2 -q 2 -F ${datafile}"
