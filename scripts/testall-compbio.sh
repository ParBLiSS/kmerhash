...
do
	...
	export OMP_NUM_THREADS=64; mpirun -np 1 ${EXEC} -b -F ${df} > ${EXEC/bin/log}.p1.t64.log 2>&1

#	echo "running 4x16 export OMP_NUM_THREADS=16; mpirun -np 4 --map-by ppr:1:socket  ${EXEC} -b -F ${df} > ${EXEC/bin/log}.p4.t16.log 2>&1"
#	export OMP_NUM_THREADS=16; mpirun -np 4 --map-by ppr:1:socket  ${EXEC} -b -F ${df} > ${EXEC/bin/log}.p4.t16.log 2>&1
...
#	EXEC=bin/testKmerCounter-${format}-a${a}-k${k}-CANONICAL-${map}-COUNT-dtIDEN-dh${dhash}-sh${shash}
#	echo "running 1x64 export OMP_NUM_THREADS=64; mpirun -np 1 ${EXEC} ${df} ${df2} > ${EXEC/bin/log}.p1.t64.log 2>&1"
