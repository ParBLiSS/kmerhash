...
protected:
	...
	inline size_t find_next_empty_pos(info_container_type const & target_info, size_t const & pos) const {
		...
		for (; (end < target_info.size()) && (target_info[end] != info_empty); ) {
			...
			end += std::max(get_distance(target_info[end]), static_cast<info_type>(1));  // move forward at least 1 (when info_normal)
		}
	...
	inline size_type find_next_non_empty_pos(info_container_type const & target_info, size_t const & pos) const {
		...
		for (; (end < target_info.size()) && !is_normal(target_info[end]); ) {
			// can skip ahead with target_info[end]
			end += std::max(get_distance(target_info[end]), static_cast<info_type>(1));  // 1 is when info is info_empty
		}
...
//      // need operation to shift empty bucket offsets.
	...
	bucket_id_type insert_with_hint(container_type & target,
		...
		while (target_info[end] != info_empty) { // loop until empty
			++(target_info[end]);  // increment
			assert(get_distance(target_info[end]) > 0);
		...
		if (end < next) {
			...
			std::cout << " infos from id " << id << " to end " << end << ": ";
			for (size_t jj = id; jj <= end; ++jj) {
				std::cout << static_cast<size_t>(target_info[jj]) << ",";
			}
			std::cout << std::endl;

			std::cout << " infos from end " << end << " to next " << next << ": ";
			for (size_t jj = end; jj <= next; ++jj) {
				std::cout << static_cast<size_t>(target_info[jj]) << ",";
			}
			std::cout << std::endl;
		}
	...
	bucket_id_type insert_with_hint_old(container_type & target,
		...
		for (size_t i = id + 1; i <= end; ++i) {
			++(target_info[i]);
			assert(get_distance(target_info[i]) > 0);
		}
	...
	template <typename InputIter, typename BucketIdIter>
	void insert_with_hint_no_resize(InputIter input, BucketIdIter bids, size_t input_size) {
		...
		for (ii = 0; ii < max_prefetch; ++ii) {
			_mm_prefetch(reinterpret_cast<const char *>(&(*(bids + ii))), _MM_HINT_T0);
		...
		for (; i < max2; ++i) {
			_mm_prefetch(reinterpret_cast<const char *>(&(*(bids + i + 2 * LOOK_AHEAD))), _MM_HINT_T0);
			// prefetch input
...
public:
	...
	void insert(Iter begin, Iter end) {
		...
		insert_with_hint_no_resize(begin, hash_vals, input_size);
		...
		while (max2 > i) {
			...
			for (; i < lmax; ++i) {
				...
				if (is_normal(info)) {
					...
					bid = id + get_distance(info);
					bid1 = id + 1 + get_distance(info_container[id + 1]);
				...
				if (info == info_empty) {  // empty bucket and no shift. insert here.
					...
					if (matched) {
						...
						while (info_container[eid] != info_empty) { // loop until empty
							...
							assert(get_distance(info_container[eid]) > 0);
						...
						if (eid < bid1) {
							...
							std::cout << " infos from end " << eid << " to next " << bid1 << ": ";
							for (size_t jj = eid; jj <= bid1; ++jj) {
								std::cout << static_cast<size_t>(info_container[jj]) << ",";
							}
							std::cout << std::endl;
						}
	...
	void insert(::std::vector<value_type> const & input) {
		...
		// now try to insert.  hashing done already.
		insert_with_hint_no_resize(input.begin(), hash_vals, input.size());
	...
	/// insert with prefetch, checks maxload each iteration
	...
	std::vector<value_type> find(Iter begin, Iter end) {
		...
		for (auto it = begin; it != end; ++it, ++it2, ++i, ++i1, ++i2) {
			...
			if (exists(found)) results.emplace_back(container[get_pos(found)]);
		}
	...
	void update(key_type const & k, mapped_type const & val) {
		...
		if (exists(bid)) {  // not inserted and no exception, so an equal entry has been found.
...
//template <typename Key, typename T, typename Hash, typename Equal, typename Allocator, typename Reducer >
//constexpr typename hashmap_robinhood_offsets_reduction<Key, T, Hash, Equal, Allocator, Reducer>::bucket_id_type hashmap_robinhood_offsets_reduction<Key, T, Hash, Equal, Allocator, Reducer>::bid_failed;
template <typename Key, typename T, typename Hash, typename Equal, typename Allocator, typename Reducer >
