...
#ifndef KMERHASH_HASHMAP_ROBINHOOD_DOUBLING_MEMMOVE_HPP_
#define KMERHASH_HASHMAP_ROBINHOOD_DOUBLING_MEMMOVE_HPP_
...
/**
 * @brief Modified robinhood hashing with doubling resizing.  modified from the "offsets" version to use memmove instead of elementwise swap.
 *
 *  TODO:
 *  [X] modification to use memmove during insertion and deletion - does not seem to make much difference for erase, so perhaps not for insert.
 *
 */
template <typename Key, typename T, typename Hash = ::std::hash<Key>,
		typename Equal = ::std::equal_to<Key>, typename Allocator = ::std::allocator<std::pair<const Key, T> > >
class hashmap_robinhood_doubling_memmove {
...
public:
     ...
     */
	explicit hashmap_robinhood_doubling_memmove(size_t const & _capacity = 128,
...
//		info_container.resize(buckets, 0x40);
	...
	hashmap_robinhood_doubling_memmove(Iter begin, Iter end,
			float const & _min_load_factor = 0.4,
			float const & _max_load_factor = 0.9) :
		hashmap_robinhood_doubling_memmove(::std::distance(begin, end) / 4, _min_load_factor, _max_load_factor) {
	...
	~hashmap_robinhood_doubling_memmove() {
...
#endif /* KMERHASH_HASHMAP_ROBINHOOD_DOUBLING_MEMMOVE_HPP_ */
